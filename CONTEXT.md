...
- **히어로 카피 사진 위로**: app/page.tsx WelcomeGuide — 사진 위 가운데 고딕(Pretendard) "오늘 뭐 입지?" + ‘그 고민은, 결에게’(결 테라코타 밑줄), radial 스크림. (이전 폴라로이드 시안은 대표님 "올드" → 클린 인플루언서로 전환한 결과)
- **상황 탭 한글화**: SituationSelector 탭이 `cat.en`→`cat.category`(일상·모임·격식·특별한 날·아이와 함께·여행·감정). 탭·상황명 폰트 Cormorant/Gowun → Pretendard
- **폰트 2종 통일**: 한글 UI=Pretendard, 영문 액센트만 Fraunces(BY SITUATION·ONEUL SERIES·01~04 번호·무드라벨). 서브카피·"오늘 어떤 자리예요?" serif→Pretendard
- **프로필 칩**: 라인198 데스크탑 전용(hidden lg:block) 분리, 모바일은 히어로 CTA 아래로 + 진단완료 시에만 노출(CTA 중복 제거). ※ 모바일 채팅 중엔 칩 미표시(트레이드오프)
- **OnulSeries**: 자기(오늘핏) 카드 제거, 컴팩트 한 줄 링크 2개(오늘무드·오늘눈치)로 축소·맨 아래
- 검증: 빌드 통과(에러·경고 0), 라이브 onulfit.com 200·hero-main.png 200. 커밋 b19a360
- **다음**: 결과 화면(코디 결과) 이미지 중심 개편 / 히어로 사진 타깃(3050) 맞게 교체 — ※ MJ 일시 차단(SFW 모더레이터가 "face cropped/몸강조" 표현에 과민, 쿨다운). 대안=Unsplash/Pexels 무료 실사 or 안전 프롬프트(로우앵글, 얼굴제거 표현 X)
- 제휴: 무신사 크리에이터 **거절**(트래픽 후 재도전). 쿠팡파트너스는 톤 안 맞아 안 붙이기로. 현 제휴=W컨셉만. 전략=②트래픽·콘텐츠 집중
